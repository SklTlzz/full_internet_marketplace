...
class Cart:
    def __init__(self, request):
        self.session = request.session # это куки 
    ...
    def add(self, product, size, quantity=1, override_quantity=False):
        ...
        '''
        c помощью cart_key мы решаем проблему c размерами. Типо размеры же разные и поэтому в корзине нам нужно
        их различать, а не приплюсовывать разные размеры к 1 товару типо они абсолютно одинаковые
        '''
    ...
    def __iter__(self):
        product_ids = [item['product_id'] for item in self.cart.values()] # перебирает все айдишники нашей корзины
    ...
    def clear(self):
        del self.session['cart'] # просто удаляем куки для корзины
        self.save()
